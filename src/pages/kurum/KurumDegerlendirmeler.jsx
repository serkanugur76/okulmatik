...
function hesaplaOrt(puanlar, kriterler) {
  if (!puanlar || !kriterler?.length) return null
  const degerler = kriterler.map(k => puanlar[k.id]).filter(v => v > 0)
...
function OrtBadge({ ort }) {
  ...
  const s = n >= 3.5 ? PUAN_BG[4] : n >= 2.5 ? PUAN_BG[3] : n >= 1.5 ? PUAN_BG[2] : PUAN_BG[1]
  return (
    <span style={{ ...s, padding: '2px 8px', borderRadius: '6px', fontWeight: '700', fontSize: '0.85rem' }}>
      {n.toFixed(2)}
    </span>
  )
}
...
export default function KurumDegerlendirmeler() {
  ...
  const [mevcut,      setMevcut]      = useState({}) // ogrenciId → {kriterId: puan}
  ...
  const [degisiklikler,   setDegisiklikler]   = useState({}) // ogrenciId → {kriterId: puan|null}
  ...
  // Mevcut değerlendirmeler (seçili rubrik + dönem)
  useEffect(() => {
  ...
  // Rubrik / dönem değişince bekleyen değişiklikleri sıfırla
  ...
  const filtreliOgrenciler = useMemo(() => {
    const liste = secilenSinifId
      ? ogrenciler.filter(o => o.sinifId === secilenSinifId)
      : ogrenciler
    // Sınıf sırasına, sonra soyada göre sırala
    return liste.slice().sort((a, b) => {
      const sa = siniflar.find(s => s.id === a.sinifId)
      const sb = siniflar.find(s => s.id === b.sinifId)
      const sevA = Number(sa?.seviye || 99)
      const sevB = Number(sb?.seviye || 99)
  ...
  // ── Puan okuma (pending > saved) ─────────────────────────
  function getPuan(ogrenciId, kriterId) {
    if (degisiklikler[ogrenciId]?.hasOwnProperty(kriterId)) return degisiklikler[ogrenciId][kriterId]
    return mevcut[ogrenciId]?.[kriterId] || null
  }

  function puanDegisti(ogrenciId, kriterId, val) {
    setDegisiklikler(prev => ({
      ...prev,
      [ogrenciId]: { ...prev[ogrenciId], [kriterId]: val ? Number(val) : null },
    }))
  }
  ...
  async function kaydet() {
    if (!hedefKurumId || !secilenRubrik || degisiklikSayisi === 0) return
    setKaydediyor(true)
    try {
      await Promise.all(
        Object.entries(degisiklikler).map(([ogrenciId, yeniPuanlar]) => {
          const birlesik = { ...(mevcut[ogrenciId] || {}) }
          Object.entries(yeniPuanlar).forEach(([kId, p]) => {
            if (p != null) birlesik[kId] = p
            else delete birlesik[kId]
          })
          const ogr = ogrenciler.find(o => o.id === ogrenciId)
          const sinif = siniflar.find(s => s.id === ogr?.sinifId)
          const ort = hesaplaOrt(birlesik, secilenRubrik.kriterler)
  ...
  function sablonIndir() {
    ...
    const kriterler = secilenRubrik.kriterler
    const baslik = ['#', 'Ad Soyad', 'TC No', 'Sınıf', ...kriterler.map(k => k.ad)]
    const satir0 = ['', 'Puan ölçeği: 1=Başlangıç  2=Gelişiyor  3=Yeterli  4=Mükemmel', '', '', ...kriterler.map(() => '1-4')]
    const rows = filtreliOgrenciler.map((ogr, i) => {
      const sinif = siniflar.find(s => s.id === ogr.sinifId)
      return [
        i + 1,
        `${ogr.ad} ${ogr.soyad}`,
        ogr.ogrenciNo || '',
        sinif?.ad || '',
        ...kriterler.map(k => getPuan(ogr.id, k.id) || ''),
      ]
    })
    const ws = XLSX.utils.aoa_to_sheet([baslik, satir0, ...rows])
    ws['!cols'] = [{ wch: 4 }, { wch: 24 }, { wch: 14 }, { wch: 10 }, ...kriterler.map(() => ({ wch: 20 }))]
    ...
    reader.onload = ev => {
      try {
        ...
        for (let i = 0; i < Math.min(rows.length, 5); i++) {
          if (rows[i].some(c => c?.toString().toUpperCase().includes('AD'))) { hIdx = i; break }
        }
        const baslik = rows[hIdx].map(c => c?.toString().trim() || '')

        // Kriter → sütun indexi eşleştir
        const kriterler = secilenRubrik?.kriterler || []
        const kriterIdx = kriterler.map(k => {
          const idx = baslik.findIndex(b => b.toLowerCase().includes(k.ad.toLowerCase().slice(0, 6)))
          return idx >= 0 ? idx : -1
        })

        // TC No sütunu
        const tcIdx = baslik.findIndex(b => b.includes('TC') || b.includes('No') || b.includes('no'))

        const satirlar = rows.slice(hIdx + 1).filter(r => r[1]?.toString().trim()).map((r, i) => {
          const adSoyad = r[1]?.toString().trim() || ''
          const tcNo = r[tcIdx]?.toString().trim() || ''
          // Öğrenci eşleştir
          ...
          const puanlar = {}
          kriterler.forEach((k, ki) => {
            const col = kriterIdx[ki]
            if (col >= 0) {
              const val = parseInt(r[col])
              if (val >= 1 && val <= 4) puanlar[k.id] = val
            }
          })
          return {
            satir: i + 1,
            ...
            tcNo,
            eslenenOgr,
            eslenmedi: !eslenenOgr,
            puanlar,
            puanSayisi: Object.keys(puanlar).length,
          }
        }).filter(s => s.adSoyad)
  ...
  async function importKaydet() {
    ...
    try {
      await Promise.all(yazilacaklar.map(s => {
        ...
        const mevcutPuanlar = mevcut[ogr.id] || {}
        const birlesik = { ...mevcutPuanlar, ...s.puanlar }
        const ort = hesaplaOrt(birlesik, secilenRubrik.kriterler)
  ...
  // ── Stiller ──────────────────────────────────────────────
  const s = {
    th:    { padding: '0.625rem 0.75rem', textAlign: 'center', fontSize: '0.72rem', fontWeight: '600', color: '#fff', background: '#1B3A6B', whiteSpace: 'nowrap' },
    thAd:  { padding: '0.625rem 0.75rem', textAlign: 'left',   fontSize: '0.72rem', fontWeight: '600', color: '#fff', background: '#1B3A6B' },
    td:    { padding: '0.5rem 0.625rem', textAlign: 'center', borderBottom: '1px solid #F1F5F9', fontSize: '0.8rem' },
    tdAd:  { padding: '0.5rem 0.75rem',  textAlign: 'left',   borderBottom: '1px solid #F1F5F9', fontSize: '0.8rem', fontWeight: '500' },
  }
            ...
            <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #E2E8F0', overflow: 'hidden' }}>
              <div style={{ padding: '0.75rem 1rem', borderBottom: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: '0.75rem', color: '#94A3B8' }}>
                  4=Mükemmel · 3=Yeterli · 2=Gelişiyor · 1=Başlangıç
                </span>
              </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
                  <thead>
                    <tr>
                      ...
                      <th style={{ ...s.thAd, minWidth: '180px' }}>Ad Soyad</th>
                      <th style={{ ...s.th, minWidth: '80px' }}>Sınıf</th>
                      {secilenRubrik.kriterler.map(k => (
                        <th key={k.id} style={{ ...s.th, minWidth: '130px', maxWidth: '180px', whiteSpace: 'normal', lineHeight: '1.3' }}>{k.ad}</th>
                      ))}
                      <th style={{ ...s.th, minWidth: '80px' }}>Dönem Ort.</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filtreliOgrenciler.length === 0 ? (
                      <tr><td colSpan={4 + secilenRubrik.kriterler.length} style={{ ...s.td, padding: '2.5rem', color: '#94A3B8' }}>Öğrenci bulunamadı</td></tr>
                    ) : filtreliOgrenciler.map((ogr, idx) => {
                      const sinif = siniflar.find(sf => sf.id === ogr.sinifId)
                      const birlesikPuanlar = {
                        ...(mevcut[ogr.id] || {}),
                        ...Object.fromEntries(
                          Object.entries(degisiklikler[ogr.id] || {}).filter(([, v]) => v != null)
                        )
                      }
                      const ort = hesaplaOrt(birlesikPuanlar, secilenRubrik.kriterler)
                      ...
                      return (
                        <tr key={ogr.id} style={{ background: degisti ? '#FFFBEB' : idx % 2 === 0 ? '#fff' : '#FAFAFA' }}>
                          ...
                          <td style={s.tdAd}>
                            {ogr.ad} {ogr.soyad}
                            {degisti && <span style={{ marginLeft: '4px', fontSize: '0.65rem', color: '#D97706', verticalAlign: 'middle' }}>●</span>}
                          </td>
                          <td style={{ ...s.td, color: '#64748B', fontSize: '0.75rem' }}>{sinif?.ad || '—'}</td>
                          {secilenRubrik.kriterler.map(k => {
                            const p = getPuan(ogr.id, k.id)
                            const stil = p ? PUAN_BG[p] : { background: '#F8FAFC', color: '#94A3B8' }
                            return (
                              <td key={k.id} style={s.td}>
                                <select
                                  value={p || ''}
                                  onChange={e => puanDegisti(ogr.id, k.id, e.target.value)}
                                  style={{
                                    width: '58px', padding: '3px 2px', borderRadius: '5px', fontSize: '0.82rem',
                                    border: '1.5px solid #E2E8F0', cursor: 'pointer', textAlign: 'center',
                          ...
                          <td style={s.td}><OrtBadge ort={ort} /></td>
                        </tr>
                      )
                    })}
                  </tbody>
            ...
            <div style={{ padding: '1.5rem' }}>
              ...
              <div style={{ background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: '10px', padding: '1rem', marginBottom: '1rem' }}>
                ...
                <div style={{ fontSize: '0.8rem', color: '#047857', marginBottom: '0.75rem' }}>
                  Şablonda öğrenci listesi ve kriter sütunları hazır gelir. Her hücreye 1–4 arası puan girin.
                </div>
                <button onClick={sablonIndir}
              ...
              <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
                <button onClick={() => setImportModal(false)} style={{ padding: '0.6rem 1.25rem', background: '#fff', border: '1.5px solid #E2E8F0', borderRadius: '8px', fontSize: '0.875rem', cursor: 'pointer', color: '#374151' }}>İptal</button>
